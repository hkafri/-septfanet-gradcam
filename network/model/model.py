...
class VAD(nn.Module):
    ...
    def forward(self, input):
        ...
        output_vad = self.forward_logits(input)
        output_vad = self.sigmoid(output_vad)

        return output_vad

    def forward_logits(self, input):
        """Return VAD logits before the sigmoid activation."""
        out_common = self.common(input)
        return self.output_layer_vad(out_common)
...
class SeparationModel(nn.Module):
    ...
    def forward(self, x: torch.float32, inference_kw={}):
        ...
        if self.final_vad and not self.final_vad_masked_speakers: #The VAD is calculated on the masks
            vad_logits = torch.cat([self.vad.forward_logits(self.mask_per_speaker[:, s]) for s in range(self.num_spk)], dim=1)
            output_vad = torch.sigmoid(vad_logits) # shape = [B, self.num_spk, T]
            self.vad_logits = vad_logits
        else:
            output_vad = 0
